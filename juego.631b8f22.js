var e={};e=import.meta.resolve("g2RJn");var t={};t=import.meta.resolve("clBo9");var a={};a=import.meta.resolve("i9Dg0");var n={};n=import.meta.resolve("6SToW");let o={data:{currentGame:{userMove:"",computerMove:""},history:JSON.parse(localStorage.getItem("jugadas")||'{"user": 0, "computer": 0}')},listeners:[],getState(){return this.data},setState(e){for(let t of(this.data=e,localStorage.setItem("jugadas",JSON.stringify(e.history)),this.listeners))t()},subscribe(e){this.listeners.push(e)},setMove(e){let t=this.getState(),a=this.getComputerMove();t.currentGame.userMove=e,t.currentGame.computerMove=a;let n=this.whoWins(e,a);"ganaste"===n?t.history.user++:"perdiste"===n&&t.history.computer++,this.setState(t)},getComputerMove:()=>["piedra","papel","tijera"][Math.floor(3*Math.random())],whoWins:(e,t)=>e===t?"empataste":"piedra"===e&&"tijera"===t||"papel"===e&&"piedra"===t||"tijera"===e&&"papel"===t?"ganaste":"perdiste"},s=[{path:/\/welcome/,component:function(e){let t=document.createElement("div");return t.innerHTML=`
    ...
    `;let c=i.querySelector("#play"),d=i.querySelector("#menu");return c&&c.addEventListener("navigate",t=>{let a=t.detail.to;console.log(a),e.goTo(`/${a}`)}),d&&d.addEventListener("navigate",t=>{let a=t.detail.to;console.log(a),e.goTo(`/${a}`)}),i}}];class r extends HTMLElement{static get observedAttributes(){return["destino","class"]}constructor(){super(),this.shadow=this.attachShadow({mode:"open"})}connectedCallback(){let e=this.getAttribute("class");this.render(e)}render(e){let t=" background: white; border: solid 3px #000; color:#000;",a=`
                 background: #fff ;
            ...
            `;let n=this.shadow.querySelector("button");n&&n.addEventListener("click",this.handleClick.bind(this))}handleClick(){let e=this.getAttribute("destino");if(e){let t=new CustomEvent("navigate",{detail:{to:e}});this.dispatchEvent(t)}}}customElements.define("my-button",r);class i extends HTMLElement{constructor(){super(),this.shadow=this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.startCountdown()}startCountdown(){let e=this.shadow.querySelector(".contador-text"),t=3;if(e){e.textContent=t.toString();let a=setInterval(()=>{t--,e.textContent=t.toString(),t<=0&&(clearInterval(a),this.dispatchEvent(new CustomEvent("countdown-finished")))},1e3)}}render(){this.shadow.innerHTML=`
                <style>
                    .contador-text {
                        font-size: 100px;
            ...
            `}}customElements.define("my-contador",i);let l=document.querySelector(".root");l&&function(e){function t(e){history.pushState({}," ",e),a(e)}function a(a){for(let n of s)if(n.path.test(a)){let a=n.component({goTo:t});e.firstChild&&e.firstChild.remove(),e.appendChild(a)}}"/"===location.pathname?t("/welcome"):a(location.pathname),window.addEventListener("popstate",()=>a(location.pathname))}(l);
//# sourceMappingURL=juego.631b8f22.js.map
